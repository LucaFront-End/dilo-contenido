...
export default function CoverSlide({ clientTitle, clientMonth, clientYear, onStart }) {
  return (
    <div className="slide-content cover-slide-container relative w-full h-full flex flex-col justify-between p-6 sm:p-8 md:p-14 overflow-hidden select-none bg-white">
      ...
      {/* Left: Authentic Single Black Swirl / Rulo emerging smaller from top-left corner (Desktop/Tablet only to avoid logo collision on mobile) */}
      <img
      ...
      {/* Main Center Area */}
      <div className="relative z-10 my-auto max-w-4xl py-4 sm:py-2 md:py-4 w-full">
        ...
        <div className="space-y-1 sm:space-y-1">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-black text-zinc-950 uppercase tracking-tight font-space leading-tight">
            PRESENTACIÓN DE
          </h2>
          <h1 className="text-4xl sm:text-5xl md:text-8xl lg:text-9xl font-black text-orange-500 uppercase tracking-tighter font-space leading-none">
            PARRILLA
          </h1>
        </div>

        {/* Dynamic Client Tagline & Month & CTA */}
        <div className="mt-6 sm:mt-6 md:mt-8 flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-3.5">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="flex-1 sm:flex-none px-3.5 py-2.5 sm:px-4 sm:py-2.5 bg-zinc-950 text-white font-bold rounded-xl md:rounded-2xl text-xs sm:text-sm md:text-base tracking-wide uppercase shadow-md flex items-center justify-center gap-1.5 border border-zinc-800">
              <Sparkles className="w-3.5 h-3.5 text-orange-400 shrink-0" />
              <span className="truncate">{clientTitle || 'Sistemas Cuauhtli'}</span>
            </div>

            <div className="px-3.5 py-2.5 sm:px-4 sm:py-2.5 bg-orange-100/90 text-orange-700 font-extrabold rounded-xl md:rounded-2xl text-xs sm:text-sm md:text-base tracking-wider uppercase border border-orange-200 text-center shrink-0">
          ...
          <button
            onClick={onStart}
            className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold rounded-xl md:rounded-2xl text-sm md:text-base transition-all shadow-md shadow-orange-500/25 flex items-center justify-center gap-2 cursor-pointer group hover:scale-[1.02] active:scale-[0.98]"
          >

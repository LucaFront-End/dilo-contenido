...
import { ArrowRight, Sparkles, ExternalLink, Search, Lock, ChevronLeft, X, Calendar, RefreshCw, CheckCircle2 } from 'lucide-react';
import { getParrillasGenerales, resolveWixMediaUrl } from '../services/wixService';

export default function HomePortal({ onSelectSlug, defaultSlug, brandFilter }) {
  const [slugInput, setSlugInput] = useState('');
  const [liveGrids, setLiveGrids] = useState([]);
  const [loading, setLoading] = useState(true);

  // Month & search filters within this client's own parrillas
  ...
  const fallbackGrids = [
    ...
    },
    {
      title: 'Sistemas Cuauhtli',
      month: 'Octubre 2026',
      slug: 'sistemas-cuauhtli-octubre-2026',
      postsCount: 20,
      hasPassword: true,
      badge: 'Parrilla Activa',
    ...
    },
    {
      title: 'Sistemas Cuauhtli',
      month: 'Agosto 2026',
      slug: 'sistemas-cuauhtli-agosto-2026',
      postsCount: 18,
      hasPassword: true,
      badge: 'Historial',
  ...
  // The client's brand is strictly isolated to their own brand (never exposes other agency clients)
  const effectiveBrand = useMemo(() => {
    return (brandFilter || sessionStorage.getItem('dilo_active_brand') || localStorage.getItem('dilo_active_brand') || 'Sistemas Cuauhtli').trim();
  }, [brandFilter]);

  // Strictly filter to ONLY grids belonging to this specific client/brand
  const clientGrids = useMemo(() => {
    const sBrand = effectiveBrand.toLowerCase();
    const filtered = allGrids.filter((g) => {
      const gTitle = (g.title || '').toLowerCase();
      return gTitle.includes(sBrand) || sBrand.includes(gTitle);
    });
    // Fallback safety if brand name has slight variations
    return filtered.length > 0 ? filtered : allGrids.filter(g => g.title?.toLowerCase().includes('cuauhtli'));
  }, [allGrids, effectiveBrand]);

  // Extract unique months for this client's editions with counts
  const monthStats = useMemo(() => {
    const stats = {};
    clientGrids.forEach((g) => {
  ...
  // Filter within this client's editions only
  const displayedGrids = useMemo(() => {
    return clientGrids.filter((g) => {
      // 1. Filter by Month
      if (selectedMonth !== 'all') {
        const gMonth = (g.month || '').toUpperCase();
        if (!gMonth.includes(selectedMonth.toUpperCase())) {
          return false;
        }
      }
  ...
  }, [clientGrids, selectedMonth, searchQuery]);
  ...
  const isFiltered = selectedMonth !== 'all' || searchQuery.trim() !== '';
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {defaultSlug && (
            <button
              ...
              title="Regresar a la presentación actual de la parrilla"
            >
      ...
      <main className="my-auto py-8 md:py-12 max-w-5xl mx-auto w-full relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-8 md:mb-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/30 text-orange-400 text-xs font-bold uppercase tracking-wider mb-4 shadow-sm">
            ...
            <span>Historial de Parrillas · {effectiveBrand}</span>
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight font-space text-white">
            Parrillas de <span className="text-orange-500">{effectiveBrand}</span>
          </h1>

          <p className="text-sm sm:text-base text-zinc-400 mt-3 sm:mt-4 leading-relaxed px-2">
            Consulta y accede a tus publicaciones mensuales anteriores y vigentes de {effectiveBrand}.
            Revisa copies, creatividades estratégicas y calendarios de cada edición.
          </p>
        </div>

        {/* Dynamic Filters Suite: Month switcher & live search for this client's editions */}
        <div className="bg-zinc-900/70 border border-zinc-800/90 rounded-3xl p-4 sm:p-6 mb-8 backdrop-blur-md shadow-2xl">
          ...
          <div className="relative mb-4">
            ...
            <input
              ...
              placeholder="Buscar en tus parrillas por mes o temática en tiempo real..."
          ...
          {/* 2. Month Filter Chips (Switch between past & current editions of this client) */}
            ...
            <div className="pt-2">
              <div className="flex items-center justify-between text-[11px] sm:text-xs text-zinc-400 font-bold uppercase tracking-wider mb-2.5">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>Ediciones Mensuales de {effectiveBrand}:</span>
                </span>
                {selectedMonth !== 'all' && (
                  <button
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 scrollbar-none no-scrollbar">
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${selectedMonth === 'all' ? 'bg-orange-700 text-white' : 'bg-zinc-800 text-zinc-400'}`}>
                    {clientGrids.length}
                  </span>
                </button>
          ...
          <div className="mt-4 pt-3 border-t border-zinc-800/60 flex items-center justify-between text-xs text-zinc-400">
            <span>
              Mostrando <strong className="text-white">{displayedGrids.length}</strong> de <strong className="text-white">{clientGrids.length}</strong> ediciones de <strong className="text-orange-400">{effectiveBrand}</strong>
        ...
        {/* Featured Grids Cards: ONLY this client's editions */}
        {displayedGrids.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto mb-10">
            {displayedGrids.map((g) => {
              const isCurrent = defaultSlug && (g.slug === defaultSlug || g.slug?.includes(defaultSlug) || defaultSlug?.includes(g.slug));
              return (
                <div
                  key={`${g.slug}-${g.month}`}
                  onClick={() => onSelectSlug(g.slug, true /* force password check */)}
                  className={`bg-zinc-900/80 hover:bg-zinc-900 border rounded-3xl p-5 sm:p-6 transition-all duration-300 shadow-xl cursor-pointer group flex flex-col justify-between ${
                    isCurrent ? 'border-orange-500/60 ring-1 ring-orange-500/20' : 'border-zinc-800 hover:border-orange-500/50'
                  }`}
                >
                  <div>
                    ...
                    <div className="aspect-video w-full rounded-2xl overflow-hidden bg-zinc-800 mb-4 border border-zinc-700/50 relative">
                      ...
                      />
                      <div className="absolute top-2.5 right-2.5 px-2.5 py-1 rounded-lg bg-black/75 backdrop-blur-md text-white text-[11px] font-bold border border-white/10 uppercase tracking-wide">
                        {g.month}
                      </div>
                    </div>
          ...
          <div className="text-center py-12 px-4 bg-zinc-900/50 border border-zinc-800 rounded-3xl max-w-xl mx-auto mb-10">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No se encontraron ediciones</h3>
            <p className="text-sm text-zinc-400 mb-5">
              No hay publicaciones coincidentes para los filtros seleccionados de {effectiveBrand}.
            </p>
            <button
              onClick={handleClearFilters}
            ...
            >
              ...
              <span>Ver todas las ediciones de {effectiveBrand}</span>
            </button>
          </div>
        )}
        ...
        <div className="max-w-md mx-auto text-center pt-4">
          <p className="text-xs text-zinc-500 mb-2 font-medium">¿Tienes un enlace o código privado de otra parrilla?</p>
          <form onSubmit={handleSearchSlugSubmit} className="relative">
            <div className="flex items-center bg-zinc-900 border border-zinc-800 rounded-2xl p-1.5 focus-within:border-orange-500 transition-colors">
              ...
              <input
                ...
                placeholder="Ingresa código o slug privado..."
